...
SELECT
    ...
    CASE
        WHEN COUNT(DISTINCT ck.claim_id) > 0 THEN
            ROUND((SUM(c.net) / COUNT(DISTINCT ck.claim_id)), 2)
        ELSE 0
...
SELECT
    ...
    SUM(c.net) - SUM(COALESCE(ra.payment_amount, 0)) as net_balance,
    ...
    CASE p_tab
        ...
        ELSE
            ...
            SELECT * FROM claims.get_doctor_denial_report(
                ...
                p_year, p_month, 'high_denial', p_limit, p_offset
            );
    END CASE;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION claims.get_doctor_denial_report IS 'Get filtered doctor denial report data for all three tabs (high_denial, summary, detail)';
