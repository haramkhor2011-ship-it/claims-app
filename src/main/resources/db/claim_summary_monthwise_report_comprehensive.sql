...
SELECT
    ...
    CASE
        WHEN SUM(ra.net) > 0 THEN
            ROUND((SUM(CASE WHEN ra.payment_amount = 0 OR ra.denial_code IS NOT NULL THEN ra.net ELSE 0 END) / SUM(ra.net)) * 100, 2)
        ELSE 0
...
BEGIN
    ...
    WITH filtered_data AS (
        SELECT
            ...
            CASE
                WHEN ra.net > 0 THEN
                    ROUND((CASE WHEN ra.payment_amount = 0 OR ra.denial_code IS NOT NULL THEN ra.net ELSE 0 END / ra.net) * 100, 2)
                ELSE 0
            END as rejected_percentage_on_remittance,
            CASE
                WHEN c.net > 0 THEN
