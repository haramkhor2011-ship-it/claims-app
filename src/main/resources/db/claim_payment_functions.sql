...
BEGIN
  ...
  SELECT 
    COALESCE(SUM(a.net), 0) as total_submitted,
    COALESCE(SUM(ra.payment_amount), 0) as total_paid,
    COALESCE(SUM(ra.net), 0) as total_remitted,
    COALESCE(SUM(CASE WHEN ra.payment_amount = 0 OR ra.denial_code IS NOT NULL THEN a.net ELSE 0 END), 0) as total_rejected,
    COALESCE(SUM(CASE WHEN ra.denial_code IS NOT NULL THEN a.net ELSE 0 END), 0) as total_denied,
