...
function Find-SrcFolder {
  ...
  while ($null -ne $dir) {
    $candidate = Join-Path $dir.FullName "soap"
...
if (-not $srcFolder) { Write-Host "[ERROR] Could not locate a 'soap' folder above $scriptDir"; exit 1 }

$projectRoot = $srcFolder.Parent.FullName
$outFile     = Join-Path $projectRoot "project_code_soap.txt"
