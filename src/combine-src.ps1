...
function Find-SrcFolder {
  ...
  while ($null -ne $dir) {
    $candidate = Join-Path $dir.FullName "src"
...
if (-not $srcFolder) { Write-Host "[ERROR] Could not locate a 'src' folder above $scriptDir"; exit 1 }

$projectRoot = $srcFolder.Parent.FullName
$outFile     = Join-Path $projectRoot "project_code.txt"
