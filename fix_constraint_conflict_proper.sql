-- Proper fix for the constraint/index naming conflict
-- This handles the case where both constraint and index exist with similar names

-- Step 1: Drop any existing constraint with 'dedup' in the name
DO $$
DECLARE
...
-- Step 3: Create the constraint with a clean name
ALTER TABLE claims.claim_event 
ADD CONSTRAINT uq_claim_event_dedup_clean 
...
WHERE constraint_name = 'uq_claim_event_dedup_clean' 
...
BEGIN
    ...
    WITH ins AS (
      ...
      ON CONFLICT ON CONSTRAINT uq_claim_event_dedup_clean DO UPDATE
